...
class WerewolfTest < Minitest::Test
  ...
  def test_when_starting_as_a_human_changing_a_second_time_it_becomes_human_again
    werewolf = Werewolf.new("David","London")
    assert werewolf.human? #true
    werewolf.change! #not human
    werewolf.change! #human
    assert werewolf.human? #should be true
  end
  ...
  def test_is_not_hungry_by_default
      werewolf = Werewolf.new("David","London")
      refute werewolf.hungry? #Fails if test is a true value

  end

  def test_becomes_hungry_after_changing_to_a_werewolf
      werewolf = Werewolf.new("David","London")
      werewolf.change! #not human = werewolf = hungry
