...
class OgreTest < Minitest::Test
  ...
  def test_it_swings_the_club_when_the_human_notices_it
    ogre = Ogre.new('Brak')
    human = Human.new
    ogre.encounter(human) #counter = 1
    assert_equal 0, ogre.swings  #swings = 0
    refute human.notices_ogre? #notices = false

    ogre.encounter(human) #counter = 2
    ogre.encounter(human) #counter = 3

    assert_equal 1, ogre.swings #should equal 1 because notice is true
    assert human.notices_ogre? #notices = true
  end
